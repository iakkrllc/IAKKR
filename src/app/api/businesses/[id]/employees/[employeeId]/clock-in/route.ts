...
export async function POST(
  ...
  const { data: openShift } = await admin
    ...
    .is("clock_out", null)
    .maybeSingle();
  if (openShift) return NextResponse.json({ error: "Already clocked in" }, { status: 400 });

  const { data, error } = await admin
    .from("employee_shifts")
    .insert({ employee_id: employeeId, business_id: businessId, created_by: auth.profile.id })
    .select("*")
    .single();
